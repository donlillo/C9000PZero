...
boolean ads1292Init(const int chipSelect, const int pwdnPin, const int startPin, const int sampleRate)
{
    uint8_t checkID;
    ...
    {
    case SPS_125:
        ads1292RegWrite(ADS1292_REG_CONFIG1, 0x00, chipSelect);
        break;
    case SPS_250:
        ads1292RegWrite(ADS1292_REG_CONFIG1, 0x01, chipSelect);
        break;
    case SPS_500:
        ads1292RegWrite(ADS1292_REG_CONFIG1, 0x02, chipSelect);
        break;
    case SPS_1k:
        ads1292RegWrite(ADS1292_REG_CONFIG1, 0x03, chipSelect);
        break;
    case SPS_2k:
        ads1292RegWrite(ADS1292_REG_CONFIG1, 0x04, chipSelect);
        break;
    case SPS_4k:
        ads1292RegWrite(ADS1292_REG_CONFIG1, 0x05, chipSelect);
        break;
    case SPS_8k:
        ads1292RegWrite(ADS1292_REG_CONFIG1, 0x06, chipSelect);    
    default:
        ads1292RegWrite(ADS1292_REG_CONFIG1, 0x00, chipSelect);
        break;
    }
    ...
    checkID = ads1292RegRead(ADS1292_REG_ID, chipSelect);
...
uint8_t ads1292RegRead(unsigned char READ_ADDRESS, const int chipSelect)
{
    uint8_t SPI_Dummy_Buff;
    uint8_t data = 0x20 + READ_ADDRESS;

    digitalWrite(chipSelect, LOW);
    delay(2);
    SPI.transfer(data);                                     //Send register location
    SPI.transfer(0x00);                                     //number of register to wr
    SPI_Dummy_Buff = SPI.transfer(CONFIG_SPI_MASTER_DUMMY); //Send value to record into register
    delay(2);
    digitalWrite(chipSelect, HIGH);
...
void ads1292StartConversion(const int chipSelect, const int startPin)
{
